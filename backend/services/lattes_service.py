...
class LattesService:
    ...
    def get_complete_resume(cls, id_lattes: str):
        inicio = time.time()
        print(f"Iniciando busca para {id_lattes}")  # Testa cache em localhost
        ...
        try:
            ...
            if not zip_bin: return {"erro": "NOT_FOUND", "detalhe": "Currículo não encontrado."}
            ...
            if root.tag == "MENSAGEM":
                detalhe = root.findtext("ERRO") or "Erro retornado pelo serviço externo."
                return {"erro": "API_ERROR", "detalhe": detalhe}
            ...
            fim = time.time()
            print(f"Tempo get_complete_resume {id_lattes}: {fim - inicio:.2f}s")
