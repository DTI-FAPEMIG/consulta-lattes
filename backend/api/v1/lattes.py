...
async def get_lattes_resume(id_lattes: str, user: dict = Depends(get_current_user)):
    ...
    if resume.get("erro") == "NOT_FOUND":
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=resume.get("detalhe", "Currículo não encontrado.")
        )

    if resume.get("erro") == "IP_DENIED":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=resume.get("detalhe", "Acesso negado.")
        )

    if resume.get("erro"):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=resume.get("detalhe", "Erro interno do servidor.")
        )
